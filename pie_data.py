...
class PIE(object):
    ...
    def get_annotated_frame_numbers(self, set_id):
        ...
        if isfile(annotated_frames_file):
            with open(annotated_frames_file, 'rt') as f:
                annotated_frames = {x.split(',')[0]: [int(fr) for fr in x.split(',')[1:]] for x in f.readlines()}
            return annotated_frames
        else:
            annotated_frames = {}
            annotation_path = join(self._annotation_path, set_id)
        
            if isdir(annotation_path):
                for v in sorted(listdir(annotation_path)):
                    if v.endswith("annt.xml") and isfile(join(annotation_path, v)):
                        video_id = v.split('_annt.xml')[0]
                        annotated_frames[video_id] = []
                    
                        path_to_file = join(annotation_path, v)
                        ...
                        for t in tracks:
                            ...
                            for b in boxes:
                                if int(b.get('outside')) == 1:
                                    continue
                                frame_id = int(b.get('frame'))
                                if frame_id not in annotated_frames[video_id]:
                                    annotated_frames[video_id].append(frame_id)
                    
                        annotated_frames[video_id].sort()
                        annotated_frames[video_id].insert(0, len(annotated_frames[video_id]))
        
            # Create the directory for the annotated frames file if it doesn't exist
            os.makedirs(os.path.dirname(annotated_frames_file), exist_ok=True)
    ...
    def get_frame_numbers(self, set_id):
        ...
        frame_ids = {v.split('_annt.xml')[0]: [] for v in sorted(listdir(join(self._annotation_path, set_id))) if v.endswith("annt.xml") and isdir(join(self._annotation_path, set_id, v.split('_annt.xml')[0]))}
    ...
    def extract_and_save_images(self, extract_frame_type='annotated'):
        set_folders = [f for f in sorted(listdir(self._clips_path)) if isdir(join(self._clips_path, f))]
            ...
            for vid, frames in sorted(extract_frames.items()):
                ...
                while success:
                    if frame_num in frames_list:
                        ...
                        img_count += 1
                        image_file_path = join(video_images_path, "%05.f.png" % frame_num)
                        # Check if the image already exists before saving
                        if not isfile(image_file_path):
    ...
    def generate_database(self):
        ...
        set_ids = [f for f in sorted(listdir(self._annotation_path)) if isdir(join(self._annotation_path, f))]
    
        database = {}
        for setid in set_ids:
            video_ids = [v.split('_annt.xml')[0] for v in sorted(listdir(join(self._annotation_path, setid))) if v.endswith("annt.xml")]
            database[setid] = {}
        ...
        for sid in set_ids:
            for vid in sorted(annotations[sid]):
                ...
                for pid in sorted(pid_annots):
                    ...
                    filtered_images, filtered_boxes, filtered_occlusions = [], [], []
                    for i in range(0, len(images), seq_stride):
                        if os.path.isfile(images[i]):
                            filtered_images.append(images[i])
                            filtered_boxes.append(boxes[i])
                            filtered_occlusions.append(occlusions[i])

                    if len(filtered_images) == 0:
                        continue

                    image_seq.append(filtered_images)
                    box_seq.append(filtered_boxes)
                    center_seq.append([self._get_center(b) for b in filtered_boxes])
                    occ_seq.append(filtered_occlusions)

                    ped_ids = [[pid]] * len(filtered_boxes)
                    pids_seq.append(ped_ids)

                    intent = [[pid_annots[pid]['attributes']['intention_prob']]] * len(filtered_boxes)
        ...
        for sid in set_ids:
            for vid in sorted(annotations[sid]):
                ...
                for pid in sorted(pid_annots):
                    ...
                    acts = [[int(pid_annots[pid]['attributes']['crossing'] > 0)]] * len(filtered_boxes)
        ...
        for sid in set_ids:
            for vid in sorted(annotations[sid]):
                ...
                for pid in sorted(pid_annots):
                    ...
                    int_prob = [[pid_annots[pid]['attributes']['intention_prob']]] * len(filtered_boxes)
                    int_bin = [[int(pid_annots[pid]['attributes']['intention_prob'] > 0.5)]] * len(filtered_boxes)
